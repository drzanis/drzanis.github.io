---
layout: archive
title: "Talks"
permalink: /talks/
---

## 🎤 Talks & Conference Presentations

Below is a curated list of my **invited talks**, and **conference presentations**, and **academic contributions** delivered at major venues such as ICSE, ARES, SIGAPP (SAC), and more.

---

### ⭐ Upcoming / Recent Talks

- **Evaluating Pre-Trained Models for Multi-Language Vulnerability Patching**  
  Presented at **ACM/SIGAPP Symposium on Applied Computing (SAC) – Computer Security (SEC) 2026**  
- **📍 Location:** *Thessaloniki, Greece*  
  Authors: Zanis Ali Khan, Aayush Garg, Yuejun Guo, Qiang Tang  

---

### 📌 Conference Presentations

- **A Multi-Dataset Evaluation of Models for Automated Vulnerability Repair**  
  *International Workshop on Artificial Intelligence, Cyber and Cyber-Physical Security (AI&CCPS), ARES 2025*  
- **📍 Location:** *Ghent, Belgium*  
  Authors: Zanis Ali Khan, Aayush Garg, Qiang Tang  
  [View on Springer Nature](https://link.springer.com/chapter/10.1007/978-3-032-00630-1_5)

- **Guidelines for Assessing the Accuracy of Log Message Template Identification Techniques**  
  *44th International Conference on Software Engineering (ICSE 2022)*  
- **📍 Location:** *Pittsburg, USA [Vitrual]*  
  Authors: Zanis Ali Khan, Donghwan Shin, Domenico Bianculli, Lionel Briand  
  [View on ACM Digital Library](https://dl.acm.org/doi/abs/10.1145/3510003.3510101)

---

### 🎓 Thesis Presentation

- **On Log Parsing and Log-based Anomaly Detection: An Empirical Evaluation**  
  *PhD Dissertation Defense, University of Luxembourg (2023)*  
  **📍 Location:** *Luxembourg, Luxembourg*  
- Author: Zanis Ali Khan  
  [OrbiLu Link](https://orbilu.uni.lu/handle/10993/60158)


---
